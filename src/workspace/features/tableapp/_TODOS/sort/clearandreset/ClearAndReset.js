...
export class ClearAndReset extends React.Component {
	state = {
    beatles: [...beatlesData],
    sort: {
      column: '',
  ...
  handleReset = () => {
    this.setState({ beatles: [...beatlesData]})  
  ...
  handleSort = (key) => {
    ...
    let arrayCopy = this.state.beatles
  ...
  handleDirection = (key) => { 
	...
	render() {
		...
		return (
			<Table>
				<Caption>
          SortBy Column Demo(Clear and Reset) <br/>
