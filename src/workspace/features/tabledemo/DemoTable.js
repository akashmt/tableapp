...
const DemoTable = props => {
  ...
  const renderPageNumbers = () => {
    return pageNumbers.map(number => {
      return (
        <li key={number} className={(number === currentPage) ? 'active' : ''} id={number} onClick={handleClick}>
          {number}
        </li>
      )
    })
  ...
  const handleOrder = (key) => {
    return direction === 'asc' && column===key ? '▲' : direction === 'desc' && column===key ? '▼' : ''
  }
  ...
  const renderTableData = () => {
    return currentAccounts.map((account, index) => {
      const { id, name, type, accountingTime, moveTime, amount, securitizationOwner } = account //destructuring
      return (
          <tr key={id}>
            <td>{name}</td>
            <td>{type}</td>
